...
use crate::{
    models::update_service::UpdateV1InstructionArgs,
    constants::*
};
...
pub async fn update_service(
    ...
    let mut new_name_input: Option<String> = None;
    match payload.get("new_name") {
        Some(new_name_option) => { 
            new_name_input = Some(new_name_option.to_string()) 
        },
        _ => ()
    }

    let mut new_uri_input: Option<String> = None;
    match payload.get("new_uri") {
        Some(new_uri_option) => { 
            new_uri_input = Some(new_uri_option.to_string()) 
        },
        _ => ()
    }

    // Construct keypairs: signer and default_keypair
    ...
    let update_service_args = UpdateV1InstructionArgs {
        new_name: new_name_input,
        new_uri: new_uri_input,
    ...
    let ix = solana_sdk::instruction::Instruction::new_with_borsh(
        ...
        &(UPDATE_SERVICE_DISCRIMINANT, update_service_args),
